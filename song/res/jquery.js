levels = localStorage.engSongLevel
total = parseInt(engSongLevelTotal);
...
$(document).ready(function () {
	...
	function Append() {
		...
		$(".option").css({
			background: 'linear-gradient(to right top, #8a61ff, #885eff, #865aff, #8357ff, #8153ff)'
		});
	...
	$('.skip-img').click(function () {
		if (coins < 10) {
			...
			coins -= 10;
			levels++; localStorage.engSongLevel = levels;
	...
	$("#ans").click(function () {
		...
		levels++;
		localStorage.engSongLevel = levels;
	});
	$(".noans").click(function () {
		...
		setTimeout(() => {
			...
			if (coins >= 4) {
				coins -= 4; localStorage.engCoins = coins;
				$("#coins").html(coins);
			} else if (coins > 0) {
				coins = 0; localStorage.engCoins = coins;
				$("#coins").html(coins);
			}
		}, 400);
	});
